...
class Player:
    ...
    def performance(self, by):
        stats = self.matches.groupby(by).sum()
        stats['appearances'] = self.matches.groupby(by)[by].count()
        stats = stats[['appearances', 'goals', 'assists']]
    ...
    def __init__(self, playerId):
        ...
        self.matches = pd.concat(dados, axis=0, ignore_index=True)
